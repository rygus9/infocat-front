...
export default function WeekScheduler({ timeScale, onChange }: WeekSchedulerParams) {
  ...
  const onBlankClick = (row: number, col: number) => {
    setStartTimes((startTimes) => startTimes.concat(indexToStartTime(row, col)));
    // 매개 변수로 받고 매개 변수를 조작하니 이런 오류가 난다.
    let [now_row, now_col] = [row, col];
    let nextTable = createTableCopy(timeTable);
    if (scale == 1) {
  ...
  const onFilledClick = (row: number, col: number) => {
    let nextTable = createTableCopy(timeTable);
    const nowType = nextTable[row][col];
    nextTable[row][col] = 0;

    if (!(nowType == 1)) {
      let [prev_row, prev_col] = prevTime(row, col);
      let [next_row, next_col] = nextTime(row, col);
      while (!(nextTable[prev_row][prev_col] === 0 || nextTable[prev_row][prev_col] === 4)) {
        nextTable[prev_row][prev_col] = 0;
      ...
      setStartTimes((startTimes) => startTimes.filter((startTime) => startTime !== indexToStartTime(time_row, time_col)));

      while (!(nextTable[next_row][next_col] === 0 || nextTable[next_row][next_col] === 3)) {
        nextTable[next_row][next_col] = 0;
        [next_row, next_col] = nextTime(next_row, next_col);
      }
    }
    setTimeTable(nextTable);
  };
